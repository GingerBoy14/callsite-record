...
// CallsiteRecord
var CallsiteRecord = function (filename, lineNum, colNum, callsiteFrameIdx, stackFrames) {
    ...
    for (var i = startLineIdx; i <= endLineIdx; i++) {
        ...
        if (i === this.lineNum){
            frameLines.push({
                num:  num,
                src:  lines[i],
                base: {line: true, underline: false }
            });
            var indicator = '';
            for (var j = 0; j<=this.colNum;j++){
                indicator += j===this.colNum ? '^' : ` `;
            }
            frameLines.push({
                num:  num,
                src:  indicator,
                base: {line: false, underline: true }
            });
        }else{
            frameLines.push({
                num:  num,
                src:  lines[i],
                base: {line: i===this.colNum, underline: false }
            });
        }
    }
    ...
    for (var i = 0; i < stackFrames.length; i++) {
        ...
        if (fnNameMatch && typeNameMatch) {
            ...
            if (callsiteFrameIdx !== null) {
                ...
                var colNum       = callsiteFrame.getColumnNumber() - 1;

                return new CallsiteRecord(filename, lineNum, colNum, callsiteFrameIdx, stackFrames);
            }
